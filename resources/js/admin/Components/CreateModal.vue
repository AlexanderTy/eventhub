<template>
    <FocusTrap>
        <div class="fixed w-screen h-screen z-40 bg-opacity-70 bg-black flex justify-center items-center">
            <form class="min-w-[500px] bg-white rounded-xl p-6" @submit.prevent="submit">
                <div class="flex gap-3 items-center">
                    <svg v-if="type === 'event'" class="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"
                         xmlns="http://www.w3.org/2000/svg">
                        <path clip-rule="evenodd"
                              d="M6.75 2.25A.75.75 0 017.5 3v1.5h9V3A.75.75 0 0118 3v1.5h.75a3 3 0 013 3v11.25a3 3 0 01-3 3H5.25a3 3 0 01-3-3V7.5a3 3 0 013-3H6V3a.75.75 0 01.75-.75zm13.5 9a1.5 1.5 0 00-1.5-1.5H5.25a1.5 1.5 0 00-1.5 1.5v7.5a1.5 1.5 0 001.5 1.5h13.5a1.5 1.5 0 001.5-1.5v-7.5z"
                              fill-rule="evenodd"/>
                    </svg>
                    <svg v-if="type === 'artist'" class="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"
                         xmlns="http://www.w3.org/2000/svg">
                        <path
                        ...
                        <path
                            d="M21.58,10.65c-.42-1.32-1.63-2.17-2.92-2.17-.3,0-.6,.04-.9,.14-1.59,.5-2.47,2.23-1.95,3.86,.41,1.32,1.62,2.17,2.92,2.17,.29,0,.6-.04,.89-.14,1.6-.5,2.47-2.23,1.96-3.86Z"></path>
                        <path
                            d="M18.99,15.2c-.45,.7-.85,1.36-1.27,2.02-.98,1.53-1.96,3.03-2.92,4.58-.12,.21-.26,.36-.42,.46-.08,.06-.17,.09-.26,.12-.09,.03-.17,.04-.27,.04-.07,0-.13,0-.21,0-.07,0-.48-.18-.87-.4-.34-.21-.16-.92-.07-1.13,.03-.08,.06-.16,.1-.24,.83-2.05,1.68-4.1,2.53-6.14,.12-.29,.24-.59,.36-.88,.01-.02,.02-.03,.06-.09,.77,1.16,1.85,1.66,3.24,1.67Z"></path>
                    </svg>
                    <svg v-if="type === 'venue'" class="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"
                         xmlns="http://www.w3.org/2000/svg">
                        <path clip-rule="evenodd"
                ...
                <div class="flex flex-col my-9 mx-1">
                    ...
                    <Input v-model="name"
                           :placeholder="type === 'event' ? 'Event title ' : type === 'artist'? 'Artist name': '' "/>
                </div>
