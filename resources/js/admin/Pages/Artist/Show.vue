<template>
    <DefaultLayout currentRoute="artists">
        <div class="flex flex-col min-w-[350px] h-full">
            ...
            <div
                class="w-full max-w-6xl z-10 flex flex-row gap-6 grid grid-cols-3"
            >
                <div class="col-span-2">
                    <ul class="flex flex-row gap-2.5 z-0">
                        ...
                        <Tab
                            ...
                            text="Social Media"
                            type="social media"
                            @tabClick="this.activeTab = 'social media'"
                        />
                        <Tab
                            :activeTab="activeTab"
                            text="SEO"
                    ...
                    <div
                        class="relative mt-[-10px] h-[450px] 2xl:h-[550px] bg-white rounded-md shadow-[7px_7px_33px_-10px_rgba(0,0,0,0.25)]"
                    >
                        <div
                            v-show="activeTab === 'general'"
                            class="absolute overflow-scroll top-0 left-0 h-full w-full p-8 flex flex-col gap-4"
                        >
                            <div class="flex flex-row gap-6">
                                <div
                                    class="text-tab-secondary w-36 h-36 rounded-full shrink-0 overflow-hidden"
                                >
                                    <img
                                        v-show="artist.image"
                                ...
                                <div>
                                    <h2 class="text-xl font-bold mb-2 mt-2.5">
                                        {{ artist.name }}
                                    </h2>
                        ...
                        <div
                            v-show="activeTab === 'social media'"
                            class="absolute top-0 overflow-scroll left-0 h-full w-full p-8 flex flex-col gap-7"
                        >
                            <div>
                                <label class="text-xs text-g mb-2"
                                    >Website url</label
                        ...
                        <div
                            ...
                            class="absolute top-0 left-0 overflow-scroll h-full w-full p-8 flex flex-col gap-7"
                        >
                            <div>
                                <label class="text-xs text-g mb-2"
                                    >SEO Title</label
                ...
                <div class="col-span-1">
                    <ul class="flex flex-row gap-2.5 z-0">
                        <Tab
                            ...
                            text="Current Events"
                            type="current"
                            @tabClick="this.secondaryActiveTab = 'current'"
                        />
                        <Tab
                            :secondaryActiveTab="secondaryActiveTab"
                            text="Previous Events"
                            type="previous"
                            @tabClick="this.secondaryActiveTab = 'previous'"
                        />
                    </ul>
                    <div
                        class="relative mt-[-10px] h-[450px] 2xl:h-[550px] bg-white rounded-md shadow-[7px_7px_33px_-10px_rgba(0,0,0,0.25)]"
                    >
                        <div
                            v-show="secondaryActiveTab === 'current'"
                            class="absolute top-0 left-0 h-full w-full p-5 flex flex-col gap-4 overflow-scroll"
                        >
                            <EventCardSmall
                                v-for="event in artist.events"
                                v-show="calcDatesLeft(event) > 0"
                                :event="event"
                            />
                        </div>
                        <!-- event.dates.some(date => date > new Date())-->
                        <div
                            v-show="secondaryActiveTab === 'previous'"
                            class="absolute top-0 left-0 h-full w-full p-5 flex flex-col gap-4 overflow-scroll"
                        >
                            <EventCardSmall
                                v-for="event in artist.events"
                        ...
                        />
                        <Teleport to="#app">
                            <CreateModal v-if="openModal" @close-modal="openModal = false" type="event" :label="'Create new event'" :artist="artist"/>
                        </Teleport>
                    </div>
                </div>
            </div>
