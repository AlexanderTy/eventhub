<template>
    ...
    :class="size === 'large' ? 'w-32 h-32' : size === 'medium' ? 'w-10 h-10' : size === 'small' ? 'w-8 h-8' : ''">
        <img v-if="artist?.image" alt="" class="w-full h-full object-cover object-top shadow-[inset_0_0_18px_rgba(0,0,0,0.1)]" :src="'/images/artists/' + artist.image"/>
        <img v-else-if="user?.image" alt="" class="w-full h-full object-cover object-top shadow-[inset_0_0_18px_rgba(0,0,0,0.1)]" :src="'/images/users/' + user.image"/>
        <svg v-else class="w-full h-full bg-white" viewBox="0 0 167 167" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
