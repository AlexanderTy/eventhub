    ...
    <div class="p-2.5 bg-white w-[16rem] h-[13rem] rounded-2xl relative shadow-[0px_8px_21px_rgba(0,0,0,0.25)]">
        ...
        <div class="pt-3 px-2 flex flex-row items-center justify-between">
            <div class="flex flex-row gap-2.5">
                <div class="bg-[url('/images/artist.png')] bg-cover w-10 h-10 rounded-full"></div>
                    <h3 class="text-sm">
                        by
                        <Link v-for="artist in event.artists" :href="$route('artists.show', { artist: artist.id })" class="text-primary font-bold hover:underline">{{ artist.name }}</Link>
                    </h3>
